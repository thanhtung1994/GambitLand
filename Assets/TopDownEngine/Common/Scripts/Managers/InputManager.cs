...
{	
	...
	public class InputManager : MMSingleton<InputManager>
	{
		...
		{
			bool isUseJoytick=true;
			IsMobile=true;
			if (GUIManager.Instance!=null)
			{
				GUIManager.Instance.SetMobileControlsActive(isUseJoytick);
				// IsMobile=false;
				if (AutoMobileDetection)
				{
					#if UNITY_ANDROID || UNITY_IPHONE
					GUIManager.Instance.SetMobileControlsActive(isUseJoytick,MovementControl);
					// IsMobile = true;
					#endif
				}
